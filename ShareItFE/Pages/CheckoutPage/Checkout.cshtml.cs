...
namespace ShareItFE.Pages.CheckoutPage
{
    public class CheckoutModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            {
                // Tải lại giỏ hàng và tổng số tiền nếu xác thực thất bại để hiển thị trang chính xác
                await OnGetAsync();
                return Page();
            }
            ...
            try
            {
                ...
                if (Input.UseSameProfile)
                {
                    ...
                    if (profileResponse.IsSuccessStatusCode)
                    {
                        ...
                        if (profile != null)
                        {
                            // Ghi đè thông tin từ profile CHỈ CHO các trường chưa được nhập
                ...
                // 1. Chuẩn bị CheckoutRequestDto dựa trên Input
                ...
                // 2. Gọi endpoint Checkout của CartController để tạo đơn hàng
                var checkoutResponse = await client.PostAsJsonAsync("api/cart/checkout", checkoutRequest);

                if (!checkoutResponse.IsSuccessStatusCode)
                {
                    var errorContent = await checkoutResponse.Content.ReadAsStringAsync();
                    ErrorMessage = $"Không thể tạo đơn hàng: {errorContent}";
                    // Tải lại giỏ hàng và tổng số tiền để hiển thị chính xác
                    await OnGetAsync();
                    return Page();
                }

                var checkoutApiResponse = JsonSerializer.Deserialize<ApiResponse<IEnumerable<OrderDto>>>(
                    await checkoutResponse.Content.ReadAsStringAsync(), options);
                var createdOrders = checkoutApiResponse?.Data?.ToList();

                if (createdOrders == null || !createdOrders.Any())
                {
                    ErrorMessage = "Không có đơn hàng nào được tạo trong quá trình thanh toán.";
                    // Tải lại giỏ hàng và tổng số tiền để hiển thị chính xác
                    await OnGetAsync();
                    return Page();
                }

                // Lấy OrderIds từ các đơn hàng đã tạo
                var orderIds = createdOrders.Select(o => o.Id).ToList();
                decimal totalAmountForPayment = createdOrders.Sum(o => o.TotalAmount); // Tính tổng từ các đơn hàng đã tạo

                // 3. Dựa trên phương thức thanh toán, gọi API thanh toán tương ứng
                if (Input.PaymentMethod == "vnpay")
                {
                    var vnpayRequest = new CreatePaymentRequestDto
                    {
                        OrderIds = orderIds,
                        Note = $"Thanh toán cho các đơn hàng: {string.Join(", ", orderIds.Select(id => id.ToString().Substring(0, 8)))}"
                    };

                    var vnpayResponse = await client.PostAsJsonAsync("api/payment/Vnpay/CreatePaymentUrl", vnpayRequest);
                    ...
                    if (vnpayResponse.IsSuccessStatusCode)
                    {
                        ...
                        if (!string.IsNullOrEmpty(paymentUrl))
                        {
                            ...
                            return Redirect(paymentUrl); // Chuyển hướng đến môi trường sandbox của VNPay
                        }
                        else
                        {
                ...
                {
                    var createTransactionRequest = new CreateTransactionRequest { OrderIds = orderIds };

                    var qrResponse = await client.PostAsJsonAsync("api/transactions/create", createTransactionRequest);
                    ...
                    if (qrResponse.IsSuccessStatusCode)
                    {
                        ...
                        if (qrApiResponse != null && qrApiResponse.Data.ValueKind != JsonValueKind.Null)
                        {
                            ...
                            // Tải lại dữ liệu trang để hiển thị đúng thông tin Order Summary cùng với mã QR
                            await OnGetAsync();
                            return Page();
                        }
                        else
            ...
            // Nếu có bất kỳ lỗi nào xảy ra, tải lại giỏ hàng và tổng số tiền và giữ nguyên trên trang hiện tại
            await OnGetAsync();
            return Page();
        }
    }
